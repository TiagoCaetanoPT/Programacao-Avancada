...
int sort_case_insensitive(const char *s1, const char *s2);
...
int main(int argc, char **argv)
{
	...
	// str_asort(vector_to_sort, 13, sort_case_insensitive);
...
int sort_case_insensitive(const char *s1, const char *s2){
	char *s1_aux = malloc(strlen(s1)+1); 
	char *s2_aux = malloc(strlen(s2)+1); 
	int res;

	strcpy(s1_aux, s1);
	strcpy(s2_aux, s2);

	string_tolower(s1_aux);
	string_tolower(s2_aux);

	res = strcmp(s1_aux, s2_aux);	
